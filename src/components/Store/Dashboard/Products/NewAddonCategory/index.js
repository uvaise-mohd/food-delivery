...
class NewAddonCategory extends Component {
    ...
    addItemCategory = (event) => {
		...
		if (this.state.store_id && this.state.type) {
            ...
            Axios.post("https://app.snakyz.com/public/api/store/add-addon-category", formData)
            .then(response => {
            ...
            <div className="bg-grey pt-50 pb-30" style={{ minHeight: '100vh' }}>
                <form onSubmit={this.addItemCategory}>
                    <div className="bg-white ml-15 mr-15 mt-20 p-15" style={{ borderRadius: '8px' }}>
                        <div>
                            <label style={{ fontWeight: '600' }}>Addon Category Name</label>
                            <input style={style_input} type="text"  value={this.state.name} onChange={this.handleInputName} placeholder="Enter Addon Category Name" required></input>	 
                        </div>
                        <div>
                            <label style={{ fontWeight: '600' }}>Addon Category Description</label>
                            <input style={{ height: '5em', border: '1px solid #D9D9D9', borderRadius: '4px', width: '100%', paddingLeft: '20px', background: 'white', color: 'black', marginBottom: '12px' }} type="text" value={this.state.description} onChange={this.handleInputDescription} placeholder="Enter Addon Category Description" required></input>	 
                        </div>
                        <div className="mt-5">
