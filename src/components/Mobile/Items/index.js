	...
	render() {
		...
		return (
			<React.Fragment>
				...
				<div style={{ minHeight: '100vh' }} className="bg-white pb-100" key={this.props.restaurant}>
					<RestaurantInfo
					...
					{this.state.couponSheet == true && (
						<React.Fragment>
							<div onClick={this.handlePopupClose} style={{ paddingLeft: '5%', paddingRight: '5%', height: '100%', width: '100%', bottom: '0px', zIndex: '9999', position: 'fixed', backgroundColor: '#000000a6' }}>
								<Fade bottom>
									<div className="bg-white" style={{ height: 'auto', left: '0', width: '100%', paddingLeft: '20px', paddingRight: '20px', paddingTop: '30px', paddingBottom: '75px', bottom: '0px', position: 'fixed', zIndex: '9999', borderTopLeftRadius: '2rem', borderTopRightRadius: '2rem' }}>
					...
					<div style={{ display: 'flex', alignItems: 'center' }} className="ml-20 mr-20">
						<div onClick={this.handleMenu} className="col text-center p-10" style={{
							fontWeight: '700', color: this.state.menu ? '#ffffff' : '#FF8585',
							backgroundColor: this.state.menu ? '#FF4646' : '#FFBCBC', letterSpacing: '1px',
							borderTopLeftRadius: '0.8rem', borderBottomLeftRadius: '0.8rem'
						}}>
							Menu
						</div>
						<div onClick={this.handleReview} className="col text-center p-10" style={{
							fontWeight: '700', color: this.state.review ? '#ffffff' : '#FF8585',
							backgroundColor: this.state.review ? '#FF4646' : '#FFBCBC', letterSpacing: '1px',
							borderTopRightRadius: '0.8rem', borderBottomRightRadius: '0.8rem'
						}}>
							Review
						</div>
					</div>
					...
					{this.state.menu && !this.state.review &&
						<React.Fragment>
							{this.props.restaurant_info && this.props.restaurant_info.filterdCoupons && this.props.restaurant_info.filterdCoupons.length > 0 &&
								<div style={{ display: 'flex' }} className="slider-wrapper ml-20 mr-20">
										...
										<div className="p-10 mr-20" style={{ border: '2px solid #FFCCCC', borderRadius: '0.5rem' }} key={index}>
											...
											<div className="mt-1" style={{ display: 'flex', color: '#7E7E7E', fontSize: '10px' }}>
												...
												{coupon.max_discount && coupon.discount_type =="PERCENTAGE" &&
													<div className="ml-1">| UPTO <span className="rupees-symbol">₹ {coupon.max_discount}</span></div>
												}
											</div>
										</div>
									))}
						...
						<React.Fragment>
							{this.props.user.success &&
								<div className="ml-20 mr-20 mt-20 mb-20">
									...
									<button
										className="mt-2"
										style={{ "backgroundColor": "rgb(255, 70, 70)", "border": "none", "width": "100%", "borderRadius": "0.5rem", "fontWeight": "bolder", "color": "white", "letterSpacing": "1px", "height": "40px" }}
