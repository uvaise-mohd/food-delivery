...
import { ArrowLeft, TimeCircle } from "react-iconly";
...
class Cart extends Component {
	...
	setSchedule = () => {
		if (this.state.date && this.state.time) {
			...
			this.state.time = this.state.time.toString().match(/^([01]\d|2[0-3])(:)([0-5]\d)(:[0-5]\d)?$/) || [this.state.time];

			if (this.state.time.length > 1) { // If time format correct
		...
		return (
			<React.Fragment>
				...
				<Dialog
					...
					PaperProps={{ style: { backgroundColor: "#fff", borderRadius: "10px", padding: '20px' } }}
				>
					<div className="text-center">
						<p>You will have to pick-Up the Order yourself from The
							Restaurant. The Order won't be Delivered to your
							Location
						</p>
						<div className="p-10" style={{
							borderRadius: "10px", fontWeight: 'bolder', color: 'white',
							letterSpacing: '1px', backgroundColor: localStorage.getItem("storeColor")
				...
				{!this.state.min_order_satisfied &&
					<div className="auth-error no-click">
						<div className="error-shake">
							...
							<span className="rupees-symbol">₹ </span>{this.props.restaurant_info.min_order_price}
						</div>
					</div>
				}
				...
				{this.state.min_qnt_message &&
					<div className="auth-error no-click">
						<div className="error-shake">
							Min cart quantity for {this.state.item.name} is {this.state.item.min_quantity}
						</div>
					</div>
				}
				...
				{this.state.max_qnt_message &&
					<div className="auth-error no-click">
						<div className="error-shake">
							Max cart quantity for {this.state.item.name} is {this.state.item.max_quantity}
						</div>
					</div>
				}
				...
				{this.state.schedule_modal == true && (
					<React.Fragment>
						<div style={{ paddingLeft: '5%', paddingRight: '5%', height: '100%', width: '100%', bottom: '0px', zIndex: '9998', position: 'fixed', backgroundColor: '#000000a6' }}>
							<Fade bottom>
								<div className="bg-white" style={{ height: 'auto', left: '0', width: '100%', paddingLeft: '20px', paddingRight: '20px', paddingTop: '20px', paddingBottom: '100px', bottom: '0px', position: 'fixed', zIndex: '9999', borderTopLeftRadius: '2rem', borderTopRightRadius: '2rem' }}>
									<div className="d-flex justify-content-end" onClick={this.handlePopupClose}>Close</div>
				...
				{!this.state.loading && (
					<React.Fragment>
						<div className={` ${localStorage.getItem("userSelected") === "DELIVERY" ? "bg-white pb-200" : "bg-white pb-100"}`}>
							<div style={{ "position": "absolute", "top": "15px", "left": "15px" }} onClick={() => this.context.router.history.goBack()}>
								<ArrowLeft />
							</div>
							<div className="text-center pt-15" style={{ "ontSize": "16px", "fontWeight": "bolder" }}>
								Cart
							</div>
							{cartProducts.length ? (
								<React.Fragment>
									...
									<div className="p-15 mt-20">
										...
										>
											<div style={{ fontSize: '15px', fontWeight: 'bolder', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} className="mb-10">
												<div>
													{restaurant_info.name}
												</div>
												<div style={{ fontWeight: '500', color: '#006FFF', fontSize: '12px' }}>
									...
									{(this.state.tipsAmountSetting || this.state.tipsPercentageSetting) && (
										<div className="mx-15 mb-15" style={{ boxShadow: 'rgb(136 136 136) 0px 0px 10px -3px', borderRadius: '10px' }}>
											{this.state.tips && this.state.is_tips_show === false && (
												<div className="block-content block-content-full bg-white pt-10 pb-5" style={{ borderRadius: '10px' }}>
													<div
														className={
													...
													>
														<p style={{ marginBottom: '2px', paddingBottom: '2px' }} className="item-text">
															<strong>Make Your Delivery Boy Happy</strong>
														</p>
														<p className="mb-0">Thank your delivery partner for helping you stay
															safe indoors. Support them through these tough
															times with a tip.</p>
														<div className="mb-2 text-muted" style={{ fontSize: '11px' }}>100% of the tip will go to your Delivery Partner</div>
													</div>
													{this.state.tipsAmountSetting && (
														<div className="tip-switch-field">
															{this.state.tips.map((tipAmount, index) => (
																<div key={index} className="tips">
																	...
																	>
																		<span className="rupees-symbol">₹ </span>{tipAmount}
																	</label>
																</div>
															))}
															<div className="tips">
									...
									{this.props.restaurant_info && this.props.restaurant_info.order_schedule == 1 &&
										<div className="mx-15 p-15" style={{ boxShadow: 'rgb(136 136 136) 0px 0px 10px -3px', borderRadius: '10px' }}>
											<div onClick={() => this.handlePopupOpen()} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
									...
									{this.props.restaurant_info && this.props.restaurant_info.order_schedule == 1 && (localStorage.getItem("schedule_date") || localStorage.getItem("schedule_time")) &&
										<div className="mt-2" style={{ color: localStorage.getItem("storeColor"), display: 'flex', justifyContent: 'center' }}>
											<span>Scheduled For:&nbsp;</span>
											{this.state.date &&
												<span><Moment format="DD/MM/YYYY">{this.state.date}</Moment>&nbsp;</span>
											}
											{this.state.time &&
									...
									<React.Fragment>
										{(this.props.restaurant_info.delivery_type == 2 || this.props.restaurant_info.delivery_type == 3) &&
											<div className="mx-15 my-15 p-15" style={{ boxShadow: 'rgb(136 136 136) 0px 0px 10px -3px', borderRadius: '10px' }}>
												<strong>Delivery Option</strong>
												<div className="mt-4" style={{ display: 'flex', justifyContent: 'space-between' }}>
													<button className="col mx-5" disabled style={{ border: '1px solid', color: localStorage.getItem("userSelected") === "DELIVERY" ? '#f50057' : '#535353', borderRadius: '2rem', backgroundColor: 'white' }}>
														<span>
															<Checkbox
																icon={<CircleUnchecked />}
																checkedIcon={<CircleChecked />}
																checked={localStorage.getItem("userSelected") === "DELIVERY" ? true : false}
																onChange={handleDelivery}
															/>
														</span>
														Delivery
													</button>
													<button className="col mx-5" style={{ border: '1px solid', color: localStorage.getItem("userSelected") === "SELFPICKUP" ? '#f50057' : '#535353', borderRadius: '2rem', backgroundColor: 'white' }}>
														<span>
															<Checkbox
																icon={<CircleUnchecked />}
																checkedIcon={<CircleChecked />}
																checked={localStorage.getItem("userSelected") === "SELFPICKUP" ? true : false}
									...
									{this.state.is_operational_loading ? (
										...
										<React.Fragment>
											{this.state.is_active ? (
												<React.Fragment>
													{(this.state.min_order_satisfied && !this.state.min_qnt_message && !this.state.max_qnt_message) && (
														<React.Fragment>
								...
								<div className="bg-white cart-empty-block" style={{ height: '70vh' }}>
									<Swing>
