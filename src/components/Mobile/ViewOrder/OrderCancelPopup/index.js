...
class OrderCancelPopup extends Component {
	...
	render() {
		...
		return (
			<React.Fragment>
				<button
					className="btn btn-square btn-outline-secondary mb-10 pl-2 pr-2 order-cancel-button mt-2"
				...
				<Modal open={this.state.open} onClose={this.handlePopupClose} closeIconSize={32}>
					<div className="text-center mt-100">
						<div style={{ fontSize: "1.2rem", fontWeight: "500" }}>
							<InfoCircle size={50} style={{ color: localStorage.getItem("storeColor") }} />
								...
								<React.Fragment>
									{order.payment_mode !== "COD" && (
										<p className="text-muted font-w400">
											<span className="rupees-symbol">₹</span>{order.total}
										</p>
									)}
									{order.payment_mode === "COD" && order.total - order.payable !== 0 && (
										<p className="text-muted font-w400">
											<span className="rupees-symbol">₹</span>{formatPrice(order.total - order.payable)}
										</p>
									)}
								</React.Fragment>
							) : (
								<p className="text-muted font-w400">
									No Refund will be made to your wallet as the restaurant has already prepared the order.	
								</p>
							)}
						</div>
						<div>
