		...
		return (
			<React.Fragment>
				<div className="mb-50 container">
					<h4 className="mb-10">Foods</h4>
					{items.map((item) => (
						<div key={item.id} className="col-xs-12 col-sm-12 restaurant-block">
							<Link
								to={{
									pathname: "/desktop/stores/" + item.store.slug + "/" + item.id,
									state: {
										fromExplorePage: true,
									},
								}}
								style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderRadius: '0.5rem' }}
								className="block-content mb-5 bg-white block-content-full pt-2"
							>
								<div>
									<div style={{ display: 'flex', alignItems: 'center' }}>
										...
										<div className="mt-2 ml-2" style={{ fontWeight: 'bolder', fontSize: '14px' }}>
											{item.name}
										</div>
									</div>
								...
								<div>
									{item.image !== null && (
										<LazyLoad>
											<img className="mt-2" style={{ borderRadius: '0.8rem', height: '90px' }} src={WEBSITE_URL + "/assets/img/items/" + item.image} alt={item.name} className="restaurant-image mt-0" />
										</LazyLoad>
									)}
								</div>
								<Ink duration="500" />
